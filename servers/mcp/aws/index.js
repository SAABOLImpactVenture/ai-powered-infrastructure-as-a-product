...
function emitEvidence(kind, status, detail) {
  const rec = { kind, status, detail, ts: new Date().toISOString(), source: 'mcp-aws' };
  const outDir = path.join(process.cwd(), 'evidence', 'mcp-aws');
...
srv.listen(PORT, () => console.log('MCP server (mcp-aws) listening on', PORT));
