...
function emitEvidence(kind, status, detail) {
  const rec = { kind, status, detail, ts: new Date().toISOString(), source: 'mcp-oci' };
  const outDir = path.join(process.cwd(), 'evidence', 'mcp-oci');
...
srv.listen(PORT, () => console.log('MCP server (mcp-oci) listening on', PORT));
