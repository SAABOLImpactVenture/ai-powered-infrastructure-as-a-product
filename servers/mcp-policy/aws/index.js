...
const srv = http.createServer(async (req, res) => {
  if (req.method === 'POST' && req.url === '/policy/check') {
    try {
      ...
      const dir = body.path || 'policies/aws/config';
      ...
      function done() {
        ...
        const payload = {
          kind: 'policy', status, ts: new Date().toISOString(), source: 'mcp-policy-aws',
          detail: (out || err).slice(0, 4000), summary, path: dir, exitCode: code
        };
        const ev = emitEvidence('mcp-policy-aws', payload);
      ...
      const payload = { kind:'policy', status:'ERROR', ts: new Date().toISOString(), source:'mcp-policy-aws', detail:String(e) };
      const ev = emitEvidence('mcp-policy-aws', payload);
...
srv.listen(PORT, () => console.log('Policy server (mcp-policy-aws) listening on', PORT));
