...
const srv = http.createServer(async (req, res) => {
  if (req.method === 'POST' && req.url === '/policy/check') {
    try {
      ...
      const dir = body.path || 'policies/oci/iam-policies';
      ...
      function done() {
        ...
        const payload = {
          kind: 'policy', status, ts: new Date().toISOString(), source: 'mcp-policy-oci',
          detail: (out || err).slice(0, 4000), summary, path: dir, exitCode: code
        };
        const ev = emitEvidence('mcp-policy-oci', payload);
      ...
      const payload = { kind:'policy', status:'ERROR', ts: new Date().toISOString(), source:'mcp-policy-oci', detail:String(e) };
      const ev = emitEvidence('mcp-policy-oci', payload);
...
srv.listen(PORT, () => console.log('Policy server (mcp-policy-oci) listening on', PORT));
