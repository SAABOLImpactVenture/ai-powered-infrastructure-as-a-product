...
The gate's authored artifacts add no invisible watermark, bidirectional or zero-width marker, prompt injection, adversarial suffix, model-disruption payload, telemetry, callback, executable hook, or runtime behavior. Its fingerprint is ordinary visible UTF-8 text. The verifier rejects control, format, nonspacing, bidirectional, zero-width, filler, and other non-rendering Unicode, all raw HTML/XML markup, Markdown embedded resources, attribute lists, active renderers, and active URI schemes on provenance surfaces. Ordinary local and HTTPS anchor links remain allowed, while provenance-authored passive browser fetches and executable Markdown extensions do not.

This source-artifact boundary is not a claim that the surrounding documentation host is network-isolated. The pre-existing MkDocs site shell declares a site-wide Mermaid presentation dependency from `unpkg.com`, so the published site injects that script on every page, including this one. The provenance artifacts neither configure nor use that dependency, and the offline verifier does not invoke MkDocs or any network service. Consumers requiring zero-network verification should use the raw repository artifacts and the local verification command above.
