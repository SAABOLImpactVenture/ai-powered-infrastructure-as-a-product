...
variable "role_def_id" { type = string }     # 8e3af657-a8ff-443c-a75c-2fe8c4bcb635 (Owner) or Contributor GUID
variable "scope" { type = string }           # /subscriptions/<sub-id>
variable "claims" { type = map(string)  # { data_class="cui", workload="payments" }
  default = {}
}

resource "azuread_application" "gha" { display_name = "gha-actions" }
resource "azuread_service_principal" "gha" { client_id = azuread_application.gha.client_id }

# Federated credentials per environment with conditional subject claim
...
# PIM/JIT: make SP eligible for role with approval required
resource "azapi_resource" "pim_eligibility" {
  ...
  name = "gha-pim-eligibility"
  body = jsonencode({
    action = "AdminAssign"
    justification = "Enable JIT via PIM for GitHub Actions SP"
