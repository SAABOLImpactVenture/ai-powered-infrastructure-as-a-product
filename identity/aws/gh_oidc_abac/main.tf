...
# ABAC roles per env; require session tag 'env' to match role name
...
# Least-privilege inline example policy with ABAC condition
resource "aws_iam_role_policy" "abac" {
  ...
  policy = jsonencode({
    ...
    Statement=[{
      ...
      Resource="*",
      Condition={ StringEquals={ "aws:PrincipalTag/env": each.key } }
    }]
  })
}

# Allow tagging session with 'env'
